 ...
 * - `dist/rtx-browser-relay-extension.zip` — packaged extension for GH releases
...
const zip = await $`zip -qr ../rtx-browser-relay-extension.zip .`.cwd(distExtension).nothrow();
if (zip.exitCode !== 0) {
...
console.log(`  ${path.join(dist, "rtx-browser-relay-extension.zip")}`);

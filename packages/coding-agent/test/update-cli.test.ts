...
describe("update-cli release binary integrity", () => {
	...
	const url = `https://github.com/therealtimex/rtx/releases/download/${tag}/${binaryName}`;
...
describe("update-cli script-shim takeover", () => {
	...
	const binaryName = "rtx-windows-x64.exe";
	...
	it("installs rtx.exe beside the shims and retires them", async () => {
		const dir = await makeTempDir();
		...
		await updateViaShimTakeover(path.join(dir, "rtx.cmd"), version, {
			binaryName,
			fetchImpl: makeFetch(exe),
			githubToken: "test-token",
		});

		expect(await Bun.file(path.join(dir, "rtx.exe")).text()).toBe(exe);
		for (const name in shims) {
	...
	it("restores the shims and removes the exe when the exe reports the wrong version", async () => {
		...
		await expect(
			updateViaShimTakeover(path.join(dir, "rtx.cmd"), version, {
				binaryName,
				fetchImpl: makeFetch(exe),
				githubToken: "test-token",
			}),
		).rejects.toThrow(/still reports 17\.2\.12 \(expected 18\.0\.0\); restored previous rtx launcher/);

		expect(await Bun.file(path.join(dir, "rtx.exe")).exists()).toBe(false);
		for (const name in shims) {
	...
	function renameLockingPs1(): Mock<typeof nodeFs.promises.rename> {
		...
		return spyOn(nodeFs.promises, "rename").mockImplementation(async (from, to) => {
			if (path.basename(String(from)) === "rtx.ps1") {
	...
	it("rewrites an immovable precedence-winning shim as a forwarder to the exe", async () => {
		...
		expect(await Bun.file(path.join(dir, "rtx.exe")).text()).toBe(exe);
		expect(await Bun.file(path.join(dir, "rtx")).exists()).toBe(false);
		expect(await Bun.file(path.join(dir, "rtx.cmd")).exists()).toBe(false);
		// PowerShell resolves .ps1 before .exe: the locked shim must now exec
		// the new binary instead of keeping its old body.
		expect(await Bun.file(path.join(dir, "rtx.ps1")).text()).toContain('& "$PSScriptRoot\\rtx.exe" @args');
	});
	...
	it("restores a forwarded shim's original body when verification fails", async () => {
		...
		try {
			...
			).rejects.toThrow("restored previous rtx launcher");
		} finally {
			renameSpy.mockRestore();
		}

	...
	try {
		const output = await Bun.build({
			...
			define: {
				...
				"process.env.RTX_BUILD_COMMIT": JSON.stringify(Bun.env.RTX_BUILD_COMMIT ?? Bun.env.GITHUB_SHA ?? ""),
				"process.env.RTX_BUILD_TAG": JSON.stringify(Bun.env.RTX_BUILD_TAG ?? Bun.env.GITHUB_REF_NAME ?? ""),

/**
 * `rtx browser-relay` implementation: serve the local CDP relay and install
...
export interface BrowserRelayCommandArgs {
	...
	port: number;
	token?: string;
	/** Install target directory; defaults to ~/.rtx/browser-relay/extension. */
	dir?: string;
	/** Gather tabs the agent actively drives into an 'rtx' Chrome tab group (default true). */
	group?: boolean;
	verbose?: boolean;
}
...
async function runInstall(dirOverride: string | undefined): Promise<void> {
	...
	console.log("  3. Enable the mode:  rtx config set browser.relay true");
	console.log("");
	console.log("rtx starts the relay automatically when the browser tool needs it;");
	console.log("run `rtx browser-relay` yourself only for --token or --no-group.");
	...
	try {
		...
		if (err instanceof Error && "code" in err && err.code === "EADDRINUSE") {
			if (await probeRelayServer(`http://127.0.0.1:${args.port}`)) {
				console.log(`rtx browser relay already running on http://127.0.0.1:${args.port}; nothing to do.`);
				return;
			}
			console.error(`Port ${args.port} is in use by something that is not an rtx browser relay.`);
			process.exit(1);
		}
		throw err;
	}

	console.log(`rtx browser relay listening on http://127.0.0.1:${args.port}`);
	console.log(`  extension endpoint  ws://127.0.0.1:${args.port}/ext${args.token ? "?token=***" : ""}`);
	if (args.port === DEFAULT_RELAY_PORT) {
		console.log("  enable with         rtx config set browser.relay true");
	} else {
		console.log(
			`  enable with         rtx config set browser.relay true && rtx config set browser.relayUrl http://127.0.0.1:${args.port}`,
		);
	}
	console.log("Waiting for the OMP Browser Relay extension to connect (rtx browser-relay install)...");

	let announced = false;
	const readiness = setInterval(() => {
		if (relay.bridge.ready && !announced) {
			announced = true;
			console.log("Extension connected. The rtx browser tool can now drive your tabs.");

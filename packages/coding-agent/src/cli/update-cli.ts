 ...
 *
 * The fork does not publish npm rename pointers, so `packages` is always
 * {@link CURRENT_PACKAGES} and the `omp.rename`-following codepaths below
 * (kept for merge parity with upstream) never trigger.
 */
export async function getLatestRelease(options: { timeoutMs?: number } = {}): Promise<ReleaseInfo> {
	...
	return { tag: release.tag, version: release.version, packages: CURRENT_PACKAGES };
}
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized rtx binary in PATH. For

...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "rtx") {
		throw new Error('Invalid alias "rtx". Refusing to shadow the base rtx command.');
	}
...
function resolveShellConfigPath(
	...
	switch (shell) {
		...
		case "fish": {
			...
			return path.join(configHome, "fish", "conf.d", "rtx-profiles.fish");
		}
		case "pwsh":
...
function renderAliasBlock(
	...
	const start = `# >>> rtx profile alias: ${aliasName} >>>`;
	const end = `# <<< rtx profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps rtx --description 'rtx profile ${profile}'`,
...
function upsertBlock(content: string, aliasName: string, block: string): string {
	const markerPairs = [
		{
			start: `# >>> rtx profile alias: ${aliasName} >>>`,
			end: `# <<< rtx profile alias: ${aliasName} <<<`,
		},
		{

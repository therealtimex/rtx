...
export function resolveCliModel(options: {
	...
	if (cliProvider && !provider) {
		return {
			...
			selector: undefined,
			warning: undefined,
			error: `Unknown provider "${cliProvider}". Run "rtx models" to see available providers/models.`,
		};
	}
	...
	if (!model) {
		...
		return {
			...
			thinkingLevel: undefined,
			warning,
			error: `Model "${display}" not found. Run "rtx models" to see available models.`,
		};
	}

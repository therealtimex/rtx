...
/**
 * `rtx ttsr` — inspect and test Time-Traveling Stream Rules.
 *
 * `rtx ttsr test` feeds a snippet (inline, --file, or stdin) through the real
 * TTSR matching pipeline and reports which rules would trigger. `rtx ttsr list`
...
export default class Ttsr extends Command {
	...
	static examples = [
		"rtx ttsr list",
		"rtx ttsr test 'const x: any = 1'",
		"rtx ttsr test src/foo.ts",
		"rtx ttsr test --file src/foo.ts",
		"rtx ttsr test --file src/foo.ts --source text",
		"rtx ttsr test --rule .rtx/rules/no-any.md --source tool --path src/foo.ts 'const x: any = 1'",
		"echo 'Box::leak(&mut v)' | rtx ttsr test --file - --path src/lib.rs",
		"rtx ttsr test --source tool --tool edit --path src/foo.ts 'const x: any = 1'",
		"rtx ttsr scan",
		"rtx ttsr scan src/",
		"rtx ttsr scan -r .rtx/rules/no-any.md src/",
	];
	...
	async run(): Promise<void> {
		...
		// inline text — so `rtx ttsr test src/foo.ts` works without --file.

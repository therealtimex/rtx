/**
 * `rtx auth-broker` — manage the rtx credential vault.
 */
...
export default class AuthBroker extends Command {
	static description = "Manage the rtx auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  rtx auth-broker serve",
		"# Boot on a non-default port\n  rtx auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  rtx auth-broker token",
		"# Rotate the bearer token\n  rtx auth-broker token --regenerate",
		"# List supported OAuth providers\n  rtx auth-broker list",
		"# Local login (run on the broker host)\n  rtx auth-broker login anthropic",
		"# Interactive provider selection\n  rtx auth-broker login",
		"# Remote login over SSH tunnel\n  rtx auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  rtx auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  rtx auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  rtx auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  rtx auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  rtx auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  rtx auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("rtx", "auth-broker", AuthBroker);
			return;
		}

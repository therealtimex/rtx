/**
 * `rtx install <target>` — top-level convenience over `rtx plugin install` /
 * `rtx plugin link`.
 *
 * The docs advertise
 *
 *   rtx install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `rtx install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: rtx install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}

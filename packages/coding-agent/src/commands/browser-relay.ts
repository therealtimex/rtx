/**
 * `rtx browser-relay` — drive the user's own Chrome tabs.
 */
...
export default class BrowserRelay extends Command {
	...
	static flags = {
		...
		dir: Flags.string({
			description: "Extension install directory (install; default ~/.rtx/browser-relay/extension)",
		}),
		"no-group": Flags.boolean({
			description: "Don't gather controllable tabs into an 'rtx' tab group",
			default: false,
		}),
	...
	static examples = [
		`${APP_NAME} browser-relay install    # write the Chrome extension to disk + setup steps`,
		`${APP_NAME} browser-relay            # serve the relay on the default port`,
		`${APP_NAME} browser-relay -p 9333 --token s3cret`,
	];

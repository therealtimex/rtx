/**
 * List and clean up agent-managed git worktrees under `~/.rtx/wt`.
 */
...
export default class Worktree extends Command {
	static description = "List or clear agent-managed git worktrees (~/.rtx/wt)";

	static aliases = ["wt"];

	static args = {
		// `list` (default) inspects the worktree dir; `clear` removes entries.
		// A positional action keeps `rtx worktree` (the no-arg form) useful.
		action: Args.string({
	...
	static examples = [
		"rtx worktree",
		"rtx worktree list --json",
		"rtx worktree clear",
		"rtx worktree clear --dry-run",
		"rtx worktree clear --all",
	];

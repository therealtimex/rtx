...
describe("profile directories", () => {
	...
	it("keeps XDG-backed named profile state under profile-specific roots", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "rtx", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "rtx", "profiles", "work"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "rtx", "profiles", "work"), { recursive: true });

		setProfile("work");

		expect(getAgentDbPath()).toBe(path.join(process.env.XDG_DATA_HOME, "rtx", "profiles", "work", "agent.db"));
		expect(getSessionsDir()).toBe(path.join(process.env.XDG_DATA_HOME, "rtx", "profiles", "work", "sessions"));
		expect(getPythonGatewayDir()).toBe(
			path.join(process.env.XDG_STATE_HOME, "rtx", "profiles", "work", "python-gateway"),
		);
	});
	...
	it("does not silently switch a named profile to XDG once the base app dir appears", async () => {
		...
		await fs.mkdir(path.join(process.env.XDG_DATA_HOME, "rtx"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_STATE_HOME, "rtx"), { recursive: true });
		await fs.mkdir(path.join(process.env.XDG_CACHE_HOME, "rtx"), { recursive: true });
	...
	it("rejects path-like profile names", () => {
		expect(() => setProfile("../work")).toThrow("Invalid rtx profile");
		expect(() => setProfile("work/team")).toThrow("Invalid rtx profile");
	});
...
describe("profile env + name validation", () => {
	...
	it("rejects uppercase profile names so isolation is filesystem-independent", () => {
		...
		expect(() => normalizeProfileName("WORK")).toThrow("Invalid rtx profile");
		expect(() => normalizeProfileName("Work")).toThrow("Invalid rtx profile");
...
describe("dirs module import behavior", () => {
	...
	it("honors XDG dir keys from a profile .env applied after the resolver froze", async () => {
		...
		try {
			...
			// Named profiles only adopt XDG when their own XDG path already exists.
			const xdgProfileRoot = path.join(xdgStateRoot, "rtx", "profiles", "work");

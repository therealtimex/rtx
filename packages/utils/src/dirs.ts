/**
 * Centralized path helpers for rtx config directories.
 *
 * Uses PI_CONFIG_DIR (default ".rtx") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/rtx/. This requires running `rtx config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, rtx trusts that the migration has been done.
 */
...
/** App name (e.g. "rtx") */
export const APP_NAME: string = "rtx";
...
/** Get the config directory name relative to home (e.g. ".rtx" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".rtx/agent" or PI_CONFIG_DIR + "/agent"). */

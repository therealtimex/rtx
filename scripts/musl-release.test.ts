...
describe("musl release artifacts", () => {
	test("builds the requested x64 and arm64 musl asset names with Bun's musl targets", async () => {
		...
		expect(result.stdout).toContain(
			"Bun.build target=bun-linux-x64-musl outfile=packages/coding-agent/binaries/rtx-linux-musl-x64",
		);
		expect(result.stdout).toContain(
			"Bun.build target=bun-linux-arm64-musl outfile=packages/coding-agent/binaries/rtx-linux-musl-arm64",
		);
	});

	test("selects the musl asset when the Linux host reports musl", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "rtx-musl-install-"));
		tempDirs.push(dir);
...
`,
		...
		expect(result.exitCode, result.stderr).toBe(0);
		expect(result.stdout).toContain("Downloading rtx-linux-musl-x64...");
		expect(await Bun.file(path.join(installDir, "rtx")).text()).toBe("binary");
	});
});

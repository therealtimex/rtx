...
smoke_cli() {
   ...
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$rtx_bin" --version
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$rtx_bin" --help >/dev/null
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$rtx_bin" stats --summary >/dev/null
   ...
   XDG_DATA_HOME="$runtime_dir/xdg" HOME="$runtime_dir/home" "$rtx_bin" --smoke-test
}
...
mkdir -p "$BINARY_DIR"
cp packages/coding-agent/dist/rtx "$BINARY_DIR/rtx"
smoke_cli "$BINARY_DIR/rtx"
...
(
   ...
   smoke_cli ./node_modules/.bin/rtx
)

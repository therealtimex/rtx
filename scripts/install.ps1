...
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/therealtimex/rtx/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/therealtimex/rtx/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/therealtimex/rtx/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/therealtimex/rtx/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/therealtimex/rtx/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
$Repo = "therealtimex/rtx"
$Package = "@oh-my-pi/pi-coding-agent"
$InstallDir = if ($env:PI_INSTALL_DIR) { $env:PI_INSTALL_DIR } else { "$env:LOCALAPPDATA\rtx" }
$BinaryName = "rtx-windows-x64.exe"
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".rtx\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host ""
            Write-Host "No bash shell found - RTX will use its built-in shell." -ForegroundColor Cyan
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("rtx-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "[OK] Installed rtx via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'rtx' to get started!"
}
...
function Install-Binary {
    ...
    Write-Host ""
    Write-Host "[OK] Installed rtx to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'rtx' to get started!"
    } else {
        Write-Host "Run 'rtx' to get started!"
    }
}

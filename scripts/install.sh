...
REPO="${RTX_REPO:-${OMP_REPO:-therealtimex/rtx}}"
...
install_via_bun() {
    ...
    echo ""
    echo "✓ Installed rtx via bun"
    echo "Run 'rtx' to get started!"
}
...
install_binary() {
    ...
    if ! SMOKE_OUTPUT="$("${INSTALL_DIR}/rtx" --version 2>&1)"; then
        echo ""
        echo "✗ rtx was downloaded to ${INSTALL_DIR}/rtx but cannot start:"
        echo "$SMOKE_OUTPUT" | sed 's/^/    /'
        if [ "$PLATFORM" = "linux-musl" ]; then
            echo ""
            echo "The musl build links libstdc++/libgcc dynamically. Install them, then re-run 'rtx':"
    ...
    # Check if in PATH
    case ":$PATH:" in
        *":$INSTALL_DIR:"*) echo "Run 'rtx' to get started!" ;;
        *) echo "Add ${INSTALL_DIR} to your PATH, then run 'rtx'" ;;
    esac
}

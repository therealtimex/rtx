#!/usr/bin/env bun
//
// Render the Homebrew formula for `rtx` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (rtx-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/rtx.rb>
//   bun scripts/ci-update-brew-formula.ts v15.10.3        # prints to stdout

import { $ } from "bun";

const REPO = process.env.OMP_REPO ?? "therealtimex/rtx";
const HOMEPAGE = "https://github.com/therealtimex/rtx";
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["rtx-*"].first` then
	// returns `nil` and `bin.install nil => "rtx"` raises.
	//
	...
	// not touch the real `/Users/<user>/.rtx` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Rtx < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/rtx-darwin-arm64",
          using: :nounzip
      sha256 "${sums["rtx-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/rtx-darwin-x64",
          using: :nounzip
      sha256 "${sums["rtx-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/rtx-linux-arm64",
          using: :nounzip
      sha256 "${sums["rtx-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/rtx-linux-x64",
          using: :nounzip
      sha256 "${sums["rtx-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["rtx-*"].first => "rtx"
    (bin/"rtx").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"rtx", "completions", shells: [:bash, :zsh, :fish])
    end
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/rtx --version")
  end
end
`;
}
...
async function main(): Promise<void> {
	...
	const targets = ["rtx-darwin-arm64", "rtx-darwin-x64", "rtx-linux-arm64", "rtx-linux-x64"];
